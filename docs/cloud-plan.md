...
AWS profile, stack and ownership tags retain the existing `synthea-cdc` names to keep resource management in the same project. The source/warehouse database is now `olist`, source schema `ecommerce`, capture prefix `raw/dms/olist`, local Compose project `olist-cdc`, and DAG `olist_cdc`.
