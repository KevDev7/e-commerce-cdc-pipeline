...
def warehouse_connection():
    return redshift_connector.connect(
        host=os.environ["REDSHIFT_HOST"], database="synthea",
...
def snapshot_table(key, prefix):
    ...
    if len(parts) == 3 and parts[0] == "healthcare" and parts[1] in TABLES and parts[2].startswith("LOAD"):
        return parts[1]
...
def check_capture():
    ...
    completed = {t["TableName"] for t in tables if t["SchemaName"] == "healthcare" and t["TableState"] == "Table completed"}
...
def load_file(connection, s3, bucket, key, role):
    ...
    try:
        ...
        if previous:
            ...
            connection.commit()
            return 0
        events = parse_csv(body.decode(), source, snapshot_table=snapshot_table(key, os.environ.get("CAPTURE_PREFIX", "capture-v1")))
        for table in TABLES:
...
def load_pending():
    ...
    prefix = os.environ.get("CAPTURE_PREFIX", "capture-v1").rstrip("/") + "/"
...
def report():
    ...
    try:
        cursor = connection.cursor()
        counts = {}
        for table in ("dim_patients", "dim_patient_history", "fct_encounters", "fct_claims", "fct_claim_transactions"):
