...
def test_freshness_requires_specific_probe_not_any_recent_file():
    class S3Fixture:
        ...
        def paginate(self, **_):
            return [{'Contents': [{'Key': 'raw/cdc/test.csv'}]}]

        def get_object(self, **_):
            return {'Body': io.BytesIO(csv_text(self.rows).encode())}

    s3 = S3Fixture()
    assert find_probe(s3, 'bucket', 'raw/cdc/', 'probe', set()) is None
    s3.rows.append(customer_row(customer_id='probe', sequence='2'))
    assert find_probe(s3, 'bucket', 'raw/cdc/', 'probe', set()) == 'raw/cdc/test.csv'
