...
from synthea_cdc import microbatch


@pytest.fixture
def capture(monkeypatch):
    objects = [{'Key': 'capture-v1/cdc/001.csv', 'ETag': 'first', 'Size': 100}]
    ...
    class S3:
        ...
        def paginate(self, **kwargs):
            assert kwargs == dict(Bucket='demo', Prefix='capture-v1/')
    ...
    monkeypatch.setenv('CAPTURE_PREFIX', 'capture-v1')
    return objects
...
def test_completed_batch_skips_until_capture_changes(tmp_path, capture):
    ...
    capture.append({'Key': 'capture-v1/cdc/002.csv', 'ETag': 'second', 'Size': 80})
